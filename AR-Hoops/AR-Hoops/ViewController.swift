...
//  AR-Hoops
//
//  Created by Dan Avram on 17/06/2018.
...
class ViewController: UIViewController, ARSCNViewDelegate {
    ...
    func shootBall() {
        ...
        ball.physicsBody?.applyForce(SCNVector3(orientation.x * power,
                                                orientation.y * power, orientation.z * power), asImpulse: true)
    ...
    func addBasket(hitTestResult: ARHitTestResult) {
        ...
        let planePos = hitTestResult.worldTransform.columns.3;
        let xPos = planePos.x
        let yPos = planePos.y
        let zPos = planePos.z
        basketNode?.position = SCNVector3(xPos, yPos, zPos)
        basketNode?.physicsBody = SCNPhysicsBody(type: .static, shape: SCNPhysicsShape(node: basketNode!, options: [SCNPhysicsShape.Option.keepAsCompound: true,
